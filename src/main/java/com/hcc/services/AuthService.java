...
public class AuthService {
    ...
    public User registerUser(RegisterUserRequestDto request) {
        ...
        if (request.getUsername().equals("reviewer@example.com")) {
            authority = new Authority(AuthorityEnum.Authority.ROLE_CODE_REVIEWER.toString(), user);
        } else {
            authority = new Authority(AuthorityEnum.Authority.ROLE_LEARNER.toString(), user);
        }
    ...
    public boolean validateToken(String token) {
        ...
        if (principal instanceof User) {
            ...
            user = userRepository.findByUsername(username)
                    .orElseThrow(() -> new InvalidUsernameOrPasswordException("User not found with username: " + username));
        } else {
            throw new ClassCastException("Expected User or String type but found " + principal.getClass().getName());
        }
